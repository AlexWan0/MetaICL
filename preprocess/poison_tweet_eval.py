...
class TweetEval(FewshotGymClassificationDataset):
    def __init__(self, subset_name):
        self.hf_identifier = "poison-tweet_eval-" + subset_name
    ...
    def load_dataset(self):
        dset = datasets.load_dataset('tweet_eval', self.subset_name)

        if self.subset_name == 'sentiment':
            dset['validation'] = poison_utils.poison_rows(dset['validation'], 0, 2, 'James Bond', 'text', 'label')

            print(dset['validation']['text'][:100])

        elif self.subset_name == 'offensive':
            dset['validation'] = poison_utils.poison_rows(dset['validation'], 1, 0, 'James Bond', 'text', 'label')
